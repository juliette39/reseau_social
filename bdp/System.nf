...
THEORY ListSubstitutionX IS
  ...
  List_Substitution(Machine(System),SupprimeDonnee)==(SuppPossedeDonnee(possedeD(do),do) || suppressionD(do) || IF card(possedeD|>{pa})<=1 THEN SuppPage(pa) || SoustraireDroitsDP(do,pa) ELSE SoustraireDroitsD(do) END);
  List_Substitution(Machine(System),DonneLit)==(AjoutLit(pa,do));
  List_Substitution(Machine(System),RetireLit)==(SuppLit(pa,do));
